# Copyright (c) 2025, Jigar Tarpara and contributors
...
def get_data(filters):
	...
	for record in sales_order_records:
		...
		row["email"] = get_customer_email(row["customer"])
		data.append(row)
...
def get_customer_email(customer):
	email = frappe.db.sql('''
		...
		INNER JOIN
			`tabContact Email` as contact_email
		ON
			contact.name = contact_email.parent	
		INNER JOIN
			`tabDynamic Link` as dynamic_table
		ON
			contact.name = dynamic_table.parent
		WHERE 
			dynamic_table.parenttype='Contact' AND
			dynamic_table.parentfield='links' AND
			dynamic_table.link_doctype='Customer' AND 
			dynamic_table.link_name=%s
